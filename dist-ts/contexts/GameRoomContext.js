...
export const GameRoomProvider = ({ children }) => {
    ...
    const createRoom = async (roomData) => {
        ...
        try {
            ...
            if (!roomData.isExhibition && roomData.bet < 500)
                throw new Error('Mise minimum : 500 FCFA');
            if (!roomData.isExhibition && roomData.bet > user.balance)
    ...
    const joinRoom = async (roomCode) => {
        ...
        try {
            ...
            if (response.room) {
                ...
                setRooms(prev => prev.map(r => r.code === roomCode ? updatedRoom : r));
    ...
    const getUserRooms = () => {
        if (!user)
            return [];
        return rooms.filter(room => room.players.some(p => p.id === user.id) &&
        ...
        const pollInterval = setInterval(async () => {
            try {
                ...
                if (roomDetails) {
                    ...
                    setRooms(prev => prev.map(r => r.code === roomCode ? roomDetails : r));
                    if (currentRoom && currentRoom.code === roomCode) {
