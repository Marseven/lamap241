...
export const useGameNotifications = () => {
    const { gameNotification, walletNotification, success, error, warning, info, } = useNotifications();
    // Notifications de début/fin de partie
    const notifyGameStart = useCallback((roomName) => {
        gameNotification(`La partie "${roomName}" commence !`, {
            duration: 4000,
            persistent: false,
        });
    }, [gameNotification]);
    const notifyGameEnd = useCallback((winner, roomName) => {
    ...
    const notifyPlayerJoined = useCallback((playerName, roomName) => {
        info(`${playerName} a rejoint "${roomName}"`, {
            duration: 4000,
            title: "Nouveau joueur",
        });
    }, [info]);
    const notifyPlayerLeft = useCallback((playerName, reason = "") => {
    ...
    const notifyPlayerReady = useCallback((playerName) => {
    ...
    const notifyGameWin = useCallback((amount) => {
    ...
    const notifyGameLoss = useCallback((amount = null) => {
        const message = amount
            ? `Vous avez perdu ${new Intl.NumberFormat("fr-FR").format(amount)} FCFA. Bonne chance pour la prochaine !`
    ...
    const notifyOpponentTurn = useCallback((opponentName) => {
        gameNotification(`${opponentName} joue...`, {
            duration: 2000,
            title: "Tour adversaire",
        });
    }, [gameNotification]);
    const notifyTimeWarning = useCallback((secondsLeft) => {
    ...
    const notifyCardPlayed = useCallback((playerName, card) => {
    ...
    const notifyRoundWin = useCallback((roundNumber) => {
        success(`🏆 Vous avez gagné la manche ${roundNumber} !`, {
            duration: 4000,
            title: "Manche gagnée",
        });
    }, [success]);
    const notifyRoundLoss = useCallback((roundNumber) => {
    ...
    const notifyWalletUpdate = useCallback((type, amount) => {
        const formattedAmount = new Intl.NumberFormat("fr-FR").format(amount);
        switch (type) {
            case "deposit":
                walletNotification(`Votre compte a été crédité de ${formattedAmount} FCFA`, {
                    duration: 6000,
                });
                break;
            case "withdraw":
    ...
    const notifyGameError = useCallback((errorMessage) => {
        error(errorMessage, {
            title: "Erreur de jeu",
            duration: 5000,
        });
    ...
    const notifyInsufficientFunds = useCallback((required, available) => {
        const requiredFormatted = new Intl.NumberFormat("fr-FR").format(required);
        const availableFormatted = new Intl.NumberFormat("fr-FR").format(available);
        error(`Solde insuffisant. Requis: ${requiredFormatted} FCFA, Disponible: ${availableFormatted} FCFA`, {
