...
export class AIDecisionMaker {
    ...
    makeDecision(aiCards, gameState) {
        const { lastCard, currentRound, roundWins, hasControl, playerCards, playerTableCards, opponentTableCards, } = gameState;
        ...
        const decision = this.selectCardWithRules(aiCards, lastCard, strategy, situation);
        // VÉRIFICATION FINALE : La carte choisie est-elle valide ?
        const isValid = playableCards.some((pc) => pc.value === decision.card.value && pc.suit === decision.card.suit);
        if (!isValid) {
    ...
    selectCardWithRules(aiCards, lastCard, strategy, situation) {
        ...
        if (sameFamily.length > 0) {
            ...
            console.log(`🤖 OBLIGATION: Doit jouer du ${lastCard.suit} (${sameFamily.length} cartes)`);
            return this.selectFromSameFamily(sameFamily, lastCard, strategy, situation);
        }
        else {
    ...
    selectFromSameFamily(sameFamily, lastCard, strategy, situation) {
        ...
        if (higherCards.length > 0) {
            ...
            if (isLastTurn || needsToWin <= 1) {
                ...
                const strongest = higherCards.reduce((max, card) => card.value > max.value ? card : max);
                return {
                    card: strongest,
                ...
                const smallest = higherCards.reduce((min, card) => card.value < min.value ? card : min);
                return {
                    card: smallest,
    ...
    analyzeSituation(aiCards, gameState) {
        ...
        return {
            ...
            needsToWin,
            hasControl,
            summary: `T${currentRound}/5, ${hasControl ? "HAS_CONTROL" : "RESPONDING"}, Threes:${threes}`,
        };
    }
    /**
    ...
    needsToWinRemainingTurns(roundWins, currentRound) {
        ...
        if (currentRound === 5)
            return 1; // Doit gagner le dernier tour
        if (currentRound === 4)
            return 2; // Doit contrôler les 2 derniers tours
